...
const JobHandleBtn = ({ id, status }) => {
  ...
  const handleAccept = () => {
    apiCall(`/api/v1/admin/jobs/${id}/accept`);
  };
  ...
  const handleModalSubmit = (e) => {
    ...
    if (modalType === "reject")
      apiCall(`/api/v1/admin/jobs/${id}/reject`, { disapprovalReason: reason });
    if (modalType === "suspend")
      apiCall(`/api/v1/admin/jobs/${id}/suspend`, {
        reasonForJobSuspension: reason,
      });
    if (modalType === "reactivate")
      apiCall(
        `/api/v1/admin/job/reactivate/${id}?reactivationReason=${reason}`,
